...
# 读取CSV文件
df = pd.read_csv('failed_names.csv')
...
for index, row in df.iterrows():
    smiles = get_smiles(row['Failed Names'], index)
...
# 将结果保存到一个新的CSV文件
df.to_csv('NIST_database_with_FAILED.csv', index=False)

# 如果有失败的名称，将它们保存到另一个CSV文件
if failed_names:
    df_failed = pd.DataFrame(failed_names, columns=['Failed Names'])
    df_failed.to_csv('failed_names2.csv', index=False)
